...
public:
	...
	Scalar computeError(const VertexSetVec& vertexSets, Scalar* chi) override
	{
		...
		return gpu::computeActiveErrors_<3>(poseEstimateData, landmarkEstimateData, d_measurements, d_omegas, d_edge2PL, d_errors, d_Xcs, chi);
	}
		...
		GpuPxPBlockVec& Hpp, GpuPx1BlockVec& bp, GpuLxLBlockVec& Hll, GpuLx1BlockVec& bl, GpuHplBlockMat& Hpl) override
    {
		...
		for (auto* vertexSet : vertexSets)
		{
			if (!vertexSet->isMarginilised())
			{
				...
				gpu::constructQuadraticForm_<3>(d_Xcs, se3_data, d_errors, d_omegas, d_edge2PL, d_edge2Hpl, d_edgeFlags, Hpp, bp, Hll, bl, Hpl);
			}
		}
    }
...
public:
	...
	Scalar computeError(const VertexSetVec& vertexSets, Scalar* chi) override
	{
		...
		return gpu::computeActiveErrors_<2>(poseEstimateData, landmarkEstimateData, d_measurements, d_omegas, d_edge2PL, d_errors, d_Xcs, chi);
	}
		...
		GpuPxPBlockVec& Hpp, GpuPx1BlockVec& bp, GpuLxLBlockVec& Hll, GpuLx1BlockVec& bl, GpuHplBlockMat& Hpl) override
    {
		...
		for (auto* vertexSet : vertexSets)
		{
			if (!vertexSet->isMarginilised())
			{
				...
				PoseVertexSet* poseVertexSet = static_cast<PoseVertexSet*>(vertexSet); 
				GpuVecSe3d se3_data = poseVertexSet->getDeviceEstimates();
				gpu::constructQuadraticForm_<2>(d_Xcs, se3_data, d_errors, d_omegas, d_edge2PL, d_edge2Hpl, d_edgeFlags, Hpp, bp, Hll, bl, Hpl);
			}
		}
    }
