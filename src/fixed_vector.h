...
{
	HOST_DEVICE Quat() {}
	HOST_DEVICE Quat(const T* values) { for (int i = 0; i < 4; i++) data[i] = values[i]; }

	template <typename U>
	HOST_DEVICE Quat(const U* values) { for (int i = 0; i < 4; i++) data[i] = T(values[i]); }

	HOST_DEVICE T& operator[](int i) { assert(i < 4); return data[i]; }
	HOST_DEVICE const T& operator[](int i) const { assert(i < 4); return data[i]; }

	HOST_DEVICE void copyTo(T* rhs) const { for (int i = 0; i < 4; i++) rhs[i] = data[i]; }

	template <typename U>
	HOST_DEVICE void copyTo(U* rhs) const { for (int i = 0; i < 4; i++) rhs[i] = U(data[i]); }
